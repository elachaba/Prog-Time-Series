...
namespace RegArchLib {
	...
	public :
		...
		cDVector	mCurrentGradLogDens ; ///< Currend gradient of conditional log density. 
	...
	public :
		...
		cRegArchGradient(cRegArchModel* theModel) ; ///< a creator
		void Delete(void) ; ///< delete
		void ReAlloc(uint theNPast, uint theNMeanParam, uint theNVarParam, uint theNDistrParameter) ;
		void ReAlloc(cRegArchModel* theModel) ;
		virtual ~cRegArchGradient() ;
		void ReInitialize(void);
		uint GetNPast(void) const ; ///< return the number of lags
		uint GetNParam(void) const ;///< return the size of the gradient vector
		uint GetNMeanParam(void) const ;///< return the number of mean parameters
		uint GetNVarParam(void) const ;///< return the number of variance parameters
		uint GetNDistrParameter(void) const ; ///< return the number of distribution parameters
		cRegArchGradient& operator=(cRegArchGradient& theSrc);
