...
namespace RegArchLib {
	...
	{
		theOut << "Const mean model with:" << endl;
		theOut << "\tConst=" << mvConst << endl;
	}
#endif // _RDLL_
	 ...
	 * \param const uint theIndex: not used here. Default 0.v
	 * \param const uint theNumParam: not used for cConst model. Default 0.
	...
	void cConst::Set(const cDVector& theVectParam, const uint theNumParam)
	{
		if (theVectParam.GetSize() > 0)
			...
			cError("cConst: theVectParam size must be > 0");
	}
	...
	{
		cDVector* myRes = new cDVector(1, mvConst);
		return *myRes;
	}
	...
	void cConst::ComputeGrad(uint theDate, const cRegArchValue& theValue, cRegArchGradient& theGradData, uint theBegIndex)
	{
