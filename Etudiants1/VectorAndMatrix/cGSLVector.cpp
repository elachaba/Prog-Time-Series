...
namespace VectorAndMatrixNameSpace
{
	...
	*/
	cGSLVector::cGSLVector(const gsl_vector* theVect)
	{
		...
		{	mvVect = gsl_vector_alloc(theVect->size) ;
			for (int i = 0 ; i < (int)theVect->size ; i++)
	...
	*/
	...
	gsl_vector* myGSLVector = theVect.GetGSLVector() ;
		...
		{	mvVect = gsl_vector_alloc(myGSLVector->size) ;
			for (int i = 0 ; i < (int)myGSLVector->size ; i++)
	...
	*/
	void cGSLVector::ReAlloc(gsl_vector* theVect)
	{
		if ((theVect != NULL))
		{
			uint mySize = (uint)(theVect->size);
			...
			else
			{
				...
				for (int i = 0; i < mySize; i++)
					gsl_vector_set(mvVect, i, (*gsl_vector_ptr(theVect, (size_t)i)));
			}
		}
		else
			Delete();
	}
	void cGSLVector::Set(double theValue, int theIndex)
	{
		if ((theIndex >= 0) && ((theIndex < (int)GetSize())))
			gsl_vector_set(mvVect, theIndex, theValue);
		else
	...
	*/
	cGSLVector& cGSLVector::operator +(double theVal) 
	{
		...
		if (mvVect != NULL)
		{	
		cGSLVector* myTmpVect = new cGSLVector(*this) ;
			*myTmpVect += theVal ;
			return *myTmpVect ;
		}
		else
	...
	*/
	...
	void cGSLVector::Print(void)
	{
		for (uint i = 0; i < GetSize(); i++)
			Rprintf("%f\n", *gsl_vector_ptr(mvVect, (size_t)i));
	...
	*/
	...
	{
	cGSLVector* myTmpVect = new cGSLVector(theVect);
		*myTmpVect += theVal ;
 		return *myTmpVect ; 
	}
	...
	*/
	...
	{
	cGSLVector* myTmpVect = new cGSLVector(theVect2);
		*myTmpVect += *theVect1 ;
		return *myTmpVect ; 
	}
	...
	*/
	...
	{
		cGSLVector* myTmpVect = new cGSLVector(theVect1);
		*myTmpVect += *theVect2;
		return *myTmpVect;
	}
	...
	*/
	cGSLVector operator -(double* theVect1, const cGSLVector& theVect2)
	{
	cGSLVector* myTmpVect = new cGSLVector(theVect2.GetSize(), *theVect1);
		*myTmpVect -= theVect2 ;
		return *myTmpVect ; 
	}
	...
	*/
	...
	{	
	uint mySize = theVect.GetSize() ;
	cGSLVector* myTmpVect = new cGSLVector(mySize, theVal);
		for (uint i = 0 ; i < mySize ; i++)
			(*myTmpVect)[i] /= theVect[i] ;
		return *myTmpVect ; 
	}
	...
	*/
	...
	{
		for (uint i = 0 ; i < theVect.GetSize() ; i++)
	...
	*/
	...
	{	
		...
		for (uint i = 1; i < theVect.GetSize(); i++)
			if (theVect[i] > myMaxi)
				myMaxi = theVect[i];
		return(myMaxi);
	}
	...
	bool IsNaN(const cGSLVector& theVect)
	{
		for (int i = 0; i < theVect.GetSize(); i++)
			if (!isfinite(theVect[i]))
				return true;
		return false;
	}
